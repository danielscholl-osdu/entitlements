...
import org.opengroup.osdu.entitlements.v2.gcp.spi.redis.operation.RemoveMemberParentUpdateOperationImpl;
...
public class RemoveMemberParentUpdateOperationImplIntegratedWithEmbededRedisTests {
    @MockBean
    ...
    public void should_updateAllChildren_whenExecute_singlePartition() throws Exception {
        ...
        commands.sadd("users.x@dp1.domain.com", "{\"role\":\"MEMBER\",\"id\":\"users.y@dp1.domain.com\",\"dataPartitionId\":\"dp1\",\"type\":\"GROUP\"}");
        ...
        Operation removeMemberParentUpdateOperation = RemoveMemberParentUpdateOperationImpl.builder().redisConnector(redisConnector)
                .retry(retry).log(log).config(config).groupNode(g2).childrenReference(ChildrenReference.createChildrenReference(g1, Role.MEMBER)).build();

        commands.select(config.getPartitionChildrenRefDb());
        removeMemberParentUpdateOperation.execute();
        assertTrue(commands.smembers("users.x@dp1.domain.com").isEmpty());
    }
    ...
    public void should_revertChanges_whenUndo_singlePartition() throws Exception {
        ...
        removeMemberParentUpdateOperation.execute();
        assertTrue(commands.smembers("users.x@dp1.domain.com").isEmpty());
        removeMemberParentUpdateOperation.undo();
        assertReferenceEquals(new String[]{"{\"role\":\"MEMBER\",\"id\":\"users.y@dp1.domain.com\",\"dataPartitionId\":\"dp1\",\"type\":\"GROUP\"}"}, commands.smembers("users.x@dp1.domain.com"));
    }
    ...
    public void should_updateAllChildren_whenExecute_crossPartition() throws Exception {
        ...
        commands1.set("users.y@dp1.domain.com", "{\"nodeId\":\"users.y@dp1.domain.com\",\"type\":\"GROUP\",\"name\":\"users.y\"," +
                "\"dataPartitionId\":\"dp1\"}");
        commands2.select(config.getPartitionChildrenRefDb());
        commands2.sadd("users.x@dp2.domain.com", "{\"role\":\"MEMBER\",\"id\":\"users.y@dp1.domain.com\",\"dataPartitionId\":\"dp1\",\"type\":\"GROUP\"}");
        ...
        commands2.select(config.getPartitionChildrenRefDb());
        assertTrue(commands2.smembers("users.x@dp2.domain.com").isEmpty());
    }
    ...
    public void should_revertChanges_whenUndo_crossPartition() throws Exception {
        ...
        assertTrue(commands2.smembers("users.x@dp2.domain.com").isEmpty());
        removeMemberParentUpdateOperation.undo();
        assertReferenceEquals(new String[]{"{\"role\":\"MEMBER\",\"id\":\"users.y@dp1.domain.com\",\"dataPartitionId\":\"dp1\",\"type\":\"GROUP\"}"}, commands2.smembers("users.x@dp2.domain.com"));
    }
